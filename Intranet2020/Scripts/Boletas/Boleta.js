...
function EjecutarScriptDetalle() {
  ...
  $("#modalAnular").on("show.bs.modal", function (event) {
    ...
    var modal = $(this);
    modal
      .find(".js-anular-nombres")
      .text(b_nombres + " " + b_apepat + " " + b_apemat);
...
function getDataFields() {
  ...
  if (!($("#bVistoCorreo").val() == "")) {
    $("#bVistoCorreo").val() == "1"
      ? (d.bVistoCorreo = true)
      : (d.bVistoCorreo = false);
    console.log(d.bVistoCorreo);
  }

  if (!($("#bCorreoEnviado").val() == "")) {
    $("#bCorreoEnviado").val() == "1"
      ? (d.bCorreoEnviado = true)
      : (d.bCorreoEnviado = false);
  }
            ...
            */
      {
        ...
        createdCell: function (td, cellData, rowData, row, col) {
          ...
          if (r[0].dFechaAnulacion == "01/01/1900") {
            $(td).html(
              '<div class="custom-control custom-checkbox">' +
                '<input type="checkbox" class="custom-control-input js-sr-checkbox" id="check_' +
                iCodBoleta +
                '" ' +
                'data-mail="' +
                vCorreo +
                ...
                vTipoBoleta +
                '">' +
                '<label class="custom-control-label js-checkbox" for="check_' +
                iCodBoleta +
                '">&nbsp;</label></div>'
            );
          } else {
...
function listarBoletasAdmin_OLD(parametros) {
  ...
  var tblBoletas = $("#datable_1").dataTable({
    ...
    fnServerData: function (url, odata, callback) {
      ...
      $.ajax({
        ...
        error: function (xhr, status, error) {
          console.log(xhr, status, error);
          console.log(
            "DataTables warning: JSON data from server failed to load or be parsed. " +
    ...
    columns: [
        ...
        createdCell: function (td, cellData, rowData, row, col) {
          ...
          if (r[0].dFechaAnulacion == "01/01/1900") {
            $(td)
              .find(".button-list")
              .append(
                '<button class="js-btn-cancel btn btn-icon btn-icon-circle btn-primary btn-icon-style-3" data-mes="' +
                  vMes +
                  '" data-anio="' +
                  vAnio +
                  '" data-dni="' +
                  ...
                  '" data-apemat="' +
                  vApeMat +
                  '" data-toggle="modal" data-target="#modalAnular"><span class="btn-icon-wrap"><i class="fa fa-power-off"></i></span></button>'
              );
          } else {
            $(td).find(".js-btn-mail").remove();
          }
      ...
      },
      {
        ...
        className: "text-center",
        title:
          '<div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input" id="customCheck1"><label class="custom-control-label js-checkbox-all" for="customCheck1">&nbsp;</label></div>',
        data: "iCodBoleta",
        createdCell: function (td, cellData, rowData, row, col) {
          let iCodBoleta = rowData.iCodBoleta,
            vAnio = rowData.vAnio,
          ...
          if (r[0].dFechaAnulacion == "01/01/1900") {
            $(td).html(
              '<div class="custom-control custom-checkbox"><input type="checkbox" class="custom-control-input js-sr-checkbox" id="check_' +
                iCodBoleta +
                '" data-mail="' +
                vCorreo +
                '" data-nombres="' +
...
function listarBoletasUsuario(parametros) {
  ...
  var tblBoletas = $("#datable_1").DataTable({
    ...
    paging: false, // Usuario no necesita paginación
    searching: false, // No busca, solo muestra sus boletas
    scrollX: true,
    processing: true,
    deferRender: true,
      ...
      $.ajax({
        ...
        success: function (response) {
          if (
            !response ||
            response.length === 0 ||
            response.vMensaje === "NO EXISTEN BOLETAS"
          ) {
            callback({ data: [], recordsTotal: 0, recordsFiltered: 0 });
          } else {
            callback({
        ...
        error: function (xhr, status, error) {
          console.error("Error DataTable Usuario:", xhr, status, error);
    ...
    columns: [
      ...
      // Acciones: solo lo que admin construyó en el backend
      {
        className: "text-center",
        title: "Acciones",
        data: "vAcciones",
...
function listarBoletasUsuario_OLD(parametros) {
  var tblBoletas = $("#datable_1").dataTable({
    bLengthChange: true,
    bFilter: false,
    processing: true,
  ...
  $(".table-boleta").on("click", ".js-btn-mail", function () {
    ...
    if (objeto.vCorreo.length !== 0) {
      $this
        .find(".btn-icon-wrap")
        .html(
          '<i class="fa fa-refresh fa-spin fa-1x fa-fw"></i><span class="sr-only"> Loading...</span >'
        );
  ...
  $.ajax({
    ...
    .done(function (response) {
      if (response.vBoletaBase64.length > 0) {
        ...
        if (opcion === "visto") {
          let iframe =
            '<iframe style="min-height:1200px;width: 100%;" src="' +
            pdf_ +
            '"></iframe>';
...
function msgSuccess2(correos) {
  ...
  $.toast({
    heading: "!Exito!",
    text:
      '<i class="jq-toast-icon fa fa-smile-o fa-1x"" aria-hidden="true"></i> Se enviaron ' +
      correos +
      " correos",
    position: "top-right",
    loaderBg: "#f68daf",
